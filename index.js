...
// // // VAD tuning. Defaults bumped slightly (threshold 0.5 -> 0.6, silence
...
// // // laptop/desktop mic + speakers setup (no headset) where the mic can pick
// // // up the device's own speaker output — exactly the setup this app runs in
// // // via the browser. Switch to "near_field" if callers use headsets.
// // const OPENAI_INPUT_NOISE_REDUCTION =
// //   process.env.OPENAI_INPUT_NOISE_REDUCTION || "far_field";

// // // ── FIX: barge-in grace period ─────────────────────────────────────────────
// // // Root cause of "agent starts and stops / never finishes a sentence" for the
// // // first couple of exchanges: without a headset, the browser mic hears the
// // // agent's own TTS audio coming out of the speakers before the platform's
// // // echo canceller has converged (this typically takes a turn or two — which
// // // matches "after two cycles it's fine"). The server VAD reads that leaked
// // // audio as the caller interrupting, cancels the in-flight response, and the
// // // agent restarts mid-thought.
// // //
// // // Fix: ignore speech_started events that arrive within BARGE_IN_GRACE_MS of
// // // a response starting to speak. A genuine human interruption essentially
// // // never happens in the first fraction of a second of the agent's turn;
// // // near-instant "speech" at that point is almost always echo, not the
// // // caller. Real interruptions after the grace window still cancel normally.
// // const BARGE_IN_GRACE_MS = Number(process.env.BARGE_IN_GRACE_MS || 600);
...
// // // ── FIX: stale-response filtering ──────────────────────────────────────────
// // // When a response gets cancelled mid-stream (barge-in), OpenAI can still
// // // deliver a handful of in-flight delta/done events for that *same*
// // // response_id after the cancel request is sent, before the cancellation
// // // actually takes effect upstream. Those leftover fragments were previously
// // // forwarded straight to ElevenLabs and spoken, overlapping with the next
// // // response — this is the other half of the "starts and stops, never
// // // finishes a sentence" bug. We tag every session with the currently "live"
// // // response_id and drop any event that doesn't match it.
// // function isStaleResponseEvent(session, event) {
// //   if (!event || !event.response_id) return false; // no id to check — let it through
// //   return event.response_id !== session.activeResponseId;
// // }
...
// //             input: {
// //               format: { type: "audio/pcm", rate: OPENAI_INPUT_SAMPLE_RATE },
// //               // FIX: hint the acoustic environment so OpenAI's own input
// //               // processing helps suppress speaker-bleed-into-mic before it
// //               // ever reaches our VAD/barge-in logic.
// //               noise_reduction: { type: OPENAI_INPUT_NOISE_REDUCTION },
// //               turn_detection: {
// //                 type: "server_vad",
// //                 threshold: OPENAI_VAD_THRESHOLD,
// //                 prefix_padding_ms: OPENAI_VAD_PREFIX_PADDING_MS,
...
// //         textBuffer: [],
// //         isResponseActive: false,
// //         // FIX: track which response is currently "live" so trailing events
// //         // from a cancelled response can be identified and dropped, and
// //         // track when the current response started speaking so we can apply
// //         // the barge-in grace period.
...
// //   // FIX: guard against a duplicate trigger (e.g. a double "start-session"
// //   // emit from the client) firing two overlapping response.create calls,
// //   // which would show up as the agent talking over/interrupting itself
// //   // right at the start of the call.
// //   if (session.greetingTriggeredMs) {
// //     console.log(
// //       `[${sessionId}] triggerGreeting called again — ignoring duplicate`,
// //     );
// //     return;
// //   }
// //   session.greetingTriggeredMs = Date.now();
...
// //       session.isResponseActive = true;
// //       session.currentAssistantText = "";
// //       // FIX: mark this as the one "live" response and note when it
// //       // started, so (a) stale events from a since-cancelled response can
// //       // be dropped, and (b) the barge-in grace period can be measured.
...
// //     case "response.output_text.delta":
// //     case "response.text.delta":
// //       if (isStaleResponseEvent(session, event)) break; // FIX: drop stale
// //       sendTextToElevenLabs(sessionId, event.delta);
// //       session.currentAssistantText += event.delta || "";
// //       session.onEvent({ type: "transcript-delta", delta: event.delta });
// //       break;
...
// //     case "response.done": {
// //       session.isResponseActive = false;
// //       // A cancelled response can still carry function_call output that
// //       // reflects an incomplete/interrupted turn — don't act on it.
// //       if (event.response?.status === "cancelled") break;
// //       const calls = extractFunctionCallsFromResponse(event.response);
// //       for (const fc of calls) await handleFunctionCall(sessionId, fc);
// //       break;
// //     }
...
// //         // FIX: barge-in grace period. Speech detected within
// //         // BARGE_IN_GRACE_MS of the agent starting to talk is almost always
// //         // the mic picking up the agent's own speaker output (echo), not a
// //         // genuine interruption — this is what caused the agent to cancel
// //         // and restart itself over and over at the start of a call. Let the
// //         // response continue; real interruptions past the grace window
// //         // still cancel normally.
// //         if (elapsedSinceResponseStart < BARGE_IN_GRACE_MS) {
// //           console.log(
// //             `[${sessionId}] Ignoring speech_started ${elapsedSinceResponseStart}ms into response (likely echo, within ${BARGE_IN_GRACE_MS}ms grace window)`,
// //           );
// //           break;
// //         }
...
// //         sendWsJson(session.ws, { type: "response.cancel" });
// //         session.isResponseActive = false;
// //         // FIX: clear activeResponseId so any trailing deltas that still
// //         // arrive for the now-cancelled response are recognized as stale
// //         // and dropped by isStaleResponseEvent() instead of being spoken.
...
// // ── FIX (v2): response staleness now tracked by an explicit "cancelled"
// // set, not by comparing against a single mutable "current" response id.
// //
// // The old approach compared event.response_id against session.activeResponseId
// // and dropped anything that didn't match. That's correct for the barge-in
// // case (activeResponseId gets nulled out right when we cancel), but it has
// // a nasty side effect on *normal* completions too: the final
// // `response.audio_transcript.done` / `response.text.done` event for a
// // response can arrive fractionally after the NEXT response has already
// // been created (activeResponseId has already moved on) — and that final,
// // most-complete piece of the transcript was being silently dropped as
// // "stale", even though nothing was actually cancelled. Meanwhile
// // ElevenLabs already had the full text (sent via earlier, non-stale
// // deltas) and spoke the whole sentence — so the caller heard the complete
// // line while the on-screen transcript stopped short.
// //
// // Fix: only drop events belonging to a response we *actually cancelled*
// // (tracked explicitly in session.cancelledResponseIds), never just because
// // a different response is now active.
// function isStaleResponseEvent(session, event) {
//   if (!event || !event.response_id) return false; // no id to check — let it through
//   return session.cancelledResponseIds.has(event.response_id);
// }
...
//         textBuffer: [],
//         isResponseActive: false,
//         // FIX: track which response is currently "live" so the barge-in
//         // grace-period calculation still works, and separately track
//         // which response ids were *actually cancelled* (see
//         // isStaleResponseEvent above) so trailing events from a genuinely
//         // cancelled response can be identified and dropped without
//         // accidentally dropping the final event of a normally-completed
//         // response.
//         activeResponseId: null,
...
//       session.isResponseActive = true;
//       session.currentAssistantText = "";
//       // FIX: mark this as the one "live" response (for barge-in timing
//       // purposes) and note when it started.
//       session.activeResponseId = event.response?.id || null;
//       session.currentResponseStartedAt = Date.now();
//       if (!session.firstResponseCreatedMs)
//         session.firstResponseCreatedMs = Date.now();
//       // FIX: tell the client a brand-new agent turn has begun so it can
//       // safely clear its running transcript display. This is the ONLY
//       // point the client should clear its displayed text — never on
//       // "done", since "done" arriving doesn't always mean the client has
//       // finished rendering every delta yet, and clearing there risked
//       // wiping the tail of a sentence (see notes on isStaleResponseEvent).
//       session.onEvent({ type: "agent-turn-start" });
//       if (
//         !session.elevenLabsWs ||
//         (session.elevenLabsWs.readyState !== WebSocket.OPEN &&
//           session.elevenLabsWs.readyState !== WebSocket.CONNECTING)
...
//     case "response.output_text.delta":
//     case "response.text.delta":
//       if (isStaleResponseEvent(session, event)) break; // FIX: drop only genuinely-cancelled
//       sendTextToElevenLabs(sessionId, event.delta);
//       session.currentAssistantText += event.delta || "";
//       session.onEvent({ type: "transcript-delta", delta: event.delta });
//       break;
...
//       if (isStaleResponseEvent(session, event)) break; // FIX: drop only genuinely-cancelled
//       flushElevenLabsStream(sessionId);
//       // FIX: prefer the server's own accumulated buffer as the source of
//       // truth — it's exactly what was streamed to ElevenLabs and spoken,
//       // so it's guaranteed to match the audio. Fall back to event.text
//       // only if for some reason nothing was accumulated.
//       const assistantText = (
//         session.currentAssistantText ||
//         event.text ||
//         ""
...
//         });
//       session.currentAssistantText = "";
//       session.onEvent({ type: "transcript-done", transcript: assistantText });
//       break;
//     }
...
//       if (isStaleResponseEvent(session, event)) break; // FIX: drop only genuinely-cancelled
//       flushElevenLabsStream(sessionId);
//       // FIX: same reasoning as above — trust the accumulated buffer first.
//       const aText = (
//         session.currentAssistantText ||
//         event.transcript ||
//         ""
...
//       session.isResponseActive = false;
//       // A cancelled response can still carry function_call output that
//       // reflects an incomplete/interrupted turn — don't act on it. Also
//       // make sure its id is recorded as cancelled so any trailing events
//       // for it are correctly dropped.
//       if (event.response?.status === "cancelled") {
//         if (event.response?.id)
//           session.cancelledResponseIds.add(event.response.id);
//         break;
//       }
//       const calls = extractFunctionCallsFromResponse(event.response);
//       for (const fc of calls) await handleFunctionCall(sessionId, fc);
...
//         sendWsJson(session.ws, { type: "response.cancel" });
//         // FIX: record this response id as genuinely cancelled BEFORE
//         // clearing activeResponseId, so isStaleResponseEvent can still
//         // correctly identify and drop its trailing events — without this
//         // explicit record, a same-id trailing event landing after
//         // activeResponseId was cleared could no longer be told apart from
//         // a late-but-legitimate event for a normally-finished response.
//         if (session.activeResponseId)
//           session.cancelledResponseIds.add(session.activeResponseId);
//         session.isResponseActive = false;
//         session.activeResponseId = null;
//         openElevenLabsStream(sessionId, true);
//       }
...
//         socket.emit("audio-clear", {});
//         break;
//       // NEW: tells the client a brand-new agent turn has started, so it
//       // can safely reset its running transcript display without risking
//       // truncation of the previous turn's text.
//       case "agent-turn-start":
//         socket.emit("agent-turn-start", {});
//         break;
//       case "transcript-delta":
//         socket.emit("transcript-delta", { delta: event.delta });
//         break;
...
const callLogSchema = new mongoose.Schema(
  {
    ...
    escalated: { type: Boolean, default: false },
    full_transcript: { type: Array, default: [] },
    // NEW: duration in seconds for analytics
...
For SERVICE DUE:
"Our records show your ${customerContext?.vehicle || "[vehicle]"} was last serviced on ${customerContext?.lastService || "[date]"}, so it's coming up due for its next scheduled maintenance. Keen to lock that in?"
Offer 2–3 concrete slots. Handle objections — acknowledge, then offer the loan car/shuttle info or a more convenient slot.
...
For OBJECTION HANDLING (practice scenario):
Expect pushback. Acknowledge the objection in one sentence, offer a genuinely lower-commitment next step.

STEP 03 — BOOKING (when the customer agrees to a slot)
Collect: which vehicle/service, preferred date, preferred time, best contact number.
Once confirmed, treat the slot as booked and include it in save_call_log.
Tell the customer: "Great, you're locked in for [date] at [time] — you'll get an SMS confirmation and calendar invite shortly."

STEP 04 — WARM TRANSFER
Trigger for: firm finance/trade-in numbers, complaints/disputes, complex negotiation.
Say: "I want to make sure you get the right numbers on this — let me get one of our consultants to give you a call."
Set outcome to "transferred_to_human" and escalated: true.
...
STEP 06 — SAVE CALL LOG (MANDATORY after every completed call)
=============================================================
After every call where purpose was established, call save_call_log.
...
- 1–2 sentences max per response
- NEVER assume caller's name — verify it
- NEVER state a firm price, interest rate, or trade-in dollar value
- ALWAYS call save_call_log after every completed call
`.trim();
}
...
function getSaveCallLogTool() {
  return {
    ...
    name: "save_call_log",
    description:
      "Saves a structured call log after every completed Patterson Cheney outbound call.",
    parameters: {
      type: "object",
      properties: {
...
function isStaleResponseEvent(session, event) {
  if (!event || !event.response_id) return false;
  return session.cancelledResponseIds.has(event.response_id);
}
...
async function generateCallSummary(transcriptArray) {
  ...
  const prompt = `You are summarizing a phone call transcript from a Patterson Cheney dealership outbound voice agent call.
...
function triggerGreeting(sessionId) {
  const session = sessions.get(sessionId);
  if (!session) return;
  if (session.greetingTriggeredMs) return;
  session.greetingTriggeredMs = Date.now();
...
function _openNewElevenLabsStream(sessionId) {
  ...
  elWs.on("open", () => {
    if (!sessions.has(sessionId) || session.elevenLabsWs !== elWs) { elWs.close(); return; }
    session.elevenLabsConnectedMs = Date.now();
    session.elevenLabsOpening = false;
...
function openElevenLabsStream(sessionId, force = false) {
  ...
  if (!force) {
    if (oldWs && (oldWs.readyState === WebSocket.OPEN || oldWs.readyState === WebSocket.CONNECTING)) return;
    _openNewElevenLabsStream(sessionId);
    return;
  }
  ...
  if (!oldWs || oldWs.readyState === WebSocket.CLOSED || oldWs.readyState === WebSocket.CLOSING) {
    session.elevenLabsWs = null;
    session.elevenLabsOpening = false;
    _openNewElevenLabsStream(sessionId);
    return;
  ...
  session.elevenLabsWs = null;
  session.elevenLabsOpening = false;
  try { oldWs.send(JSON.stringify({ text: "" })); } catch (_) {}
  oldWs.once("close", () => {
    if (!sessions.has(sessionId)) return;
    const s = sessions.get(sessionId);
    if (s && !s.elevenLabsWs && !s.elevenLabsOpening) _openNewElevenLabsStream(sessionId);
  });
  setTimeout(() => {
    if (!sessions.has(sessionId)) return;
    const s = sessions.get(sessionId);
...
function sendTextToElevenLabs(sessionId, text) {
  const session = sessions.get(sessionId);
  if (!session) return;
  if (session.elevenLabsWs?.readyState === WebSocket.OPEN && session.elevenLabsReady) {
...
async function createMockBooking({ sessionId, customer_name, phone, vehicle, brand, type, date, time, notes }) {
  const booking_ref = generateBookingRef();
  const doc = new MockBooking({
    id: uuidv4(),
    booking_ref,
...
async function handleFunctionCall(sessionId, eventOrItem) {
  ...
  try {
    ...
    const logId = uuidv4();
    const durationSeconds = args.duration_seconds || session.recorder.getDurationSeconds();
    ...
    const callLog = new CallLog({
      ...
      caller_phone: args.caller_phone || null,
      caller_email: args.caller_email || null,
      vehicle_interest: args.vehicle_interest || session.customerContext?.vehicle || null,
      scenario: session.customerContext?.scenario || null,
      intent_category: args.intent_category,
      outcome: args.outcome,
      booking: bookingRecord
        ? { date: bookingRecord.date, time: bookingRecord.time, type: bookingRecord.type, booking_ref: bookingRecord.booking_ref }
        : { date: null, time: null, type: null, booking_ref: null },
      upgrade_interest_level: args.upgrade_interest_level || null,
      objections: Array.isArray(args.objections) ? args.objections : [],
      recommended_next_action: args.recommended_next_action || null,
...
async function handleRealtimeEvent(sessionId, event) {
  ...
  switch (event.type) {
    ...
    case "response.created":
      ...
      session.activeResponseId = event.response?.id || null;
      session.currentResponseStartedAt = Date.now();
      if (!session.firstResponseCreatedMs) session.firstResponseCreatedMs = Date.now();
      session.onEvent({ type: "agent-turn-start" });
      if (!session.elevenLabsWs || (session.elevenLabsWs.readyState !== WebSocket.OPEN && session.elevenLabsWs.readyState !== WebSocket.CONNECTING)) {
        openElevenLabsStream(sessionId);
      }
      break;
    ...
    case "response.output_text.delta":
    case "response.text.delta":
      if (isStaleResponseEvent(session, event)) break;
      sendTextToElevenLabs(sessionId, event.delta);
      session.currentAssistantText += event.delta || "";
      session.onEvent({ type: "transcript-delta", delta: event.delta });
      break;
    ...
    case "response.text.done": {
      if (isStaleResponseEvent(session, event)) break;
      flushElevenLabsStream(sessionId);
      const assistantText = (session.currentAssistantText || event.text || "").trim();
      if (assistantText) session.transcript.push({ role: "assistant", text: assistantText, ts: Date.now() });
      session.currentAssistantText = "";
      session.onEvent({ type: "transcript-done", transcript: assistantText });
      break;
    }
    ...
    case "response.audio_transcript.done": {
      if (isStaleResponseEvent(session, event)) break;
      flushElevenLabsStream(sessionId);
      const aText = (session.currentAssistantText || event.transcript || "").trim();
      if (aText) session.transcript.push({ role: "assistant", text: aText, ts: Date.now() });
      session.currentAssistantText = "";
      session.onEvent({ type: "transcript-done", transcript: aText });
      break;
    }
    ...
    case "response.done": {
      session.isResponseActive = false;
      if (event.response?.status === "cancelled") {
        if (event.response?.id) session.cancelledResponseIds.add(event.response.id);
        break;
      }
      const calls = extractFunctionCallsFromResponse(event.response);
      for (const fc of calls) await handleFunctionCall(sessionId, fc);
    ...
    case "input_audio_buffer.speech_started": {
      if (session.isResponseActive) {
        ...
        sendWsJson(session.ws, { type: "response.cancel" });
        if (session.activeResponseId) session.cancelledResponseIds.add(session.activeResponseId);
        session.isResponseActive = false;
        session.activeResponseId = null;
        openElevenLabsStream(sessionId, true);
      }
    ...
    case "conversation.item.input_audio_transcription.completed":
      if (event.transcript && event.transcript.trim()) {
        session.transcript.push({ role: "user", text: event.transcript.trim(), ts: Date.now() });
      }
      session.onEvent({ type: "user-transcript", transcript: event.transcript });
      break;

    case "error":
      if (event.error && event.error.code === "response_cancel_not_active") break;
...
async function closeSession(sessionId) {
  ...
  if (hadConversation) {
    ...
    try {
      const summaryData = await generateCallSummary(session.transcript);
      session.onEvent({ type: "call-summary", data: { ...summaryData, fullTranscript: session.transcript } });
      ...
      if (session.callLogs.length > 0) {
        await CallLog.findOneAndUpdate(
          { sessionId },
          {
            $set: {
              ...
              ai_summary: summaryData.summary || undefined,
              sentiment: summaryData.sentiment || undefined,
              upgrade_interest_level: summaryData.upgrade_interest_level || undefined,
              objections: summaryData.objections || undefined,
              recommended_next_action: summaryData.recommended_next_action || undefined,
              duration_seconds: session.recorder.getDurationSeconds(),
            },
          },
          { sort: { createdAt: -1 }, new: true },
        );
        ...
        if (summaryData.booking?.date && summaryData.booking?.time) {
          bookingRecord = await createMockBooking({
            sessionId,
            customer_name: summaryData.caller_name || session.customerContext?.name || "Unknown",
            vehicle: summaryData.vehicle_interest || session.customerContext?.vehicle,
            brand: session.customerContext?.brand,
            type: summaryData.booking.type || "service",
            date: summaryData.booking.date,
            time: summaryData.booking.time,
        ...
        const callLog = new CallLog({
          ...
          brand: session.customerContext?.brand || null,
          caller_name: summaryData.caller_name || null,
          vehicle_interest: summaryData.vehicle_interest || session.customerContext?.vehicle || null,
          scenario: session.customerContext?.scenario || null,
          intent_category: summaryData.intent_category || "no_transcript_admin",
          outcome: summaryData.outcome || "message_taken",
          booking: bookingRecord
      ...
      console.error(`[${sessionId}] Summary generation/save failed:`, err.message);
    }
  }
...
function buildEventForwarder(socket) {
  return (event) => {
    switch (event.type) {
      ...
      case "clear-audio": socket.emit("audio-clear", {}); break;
      case "agent-turn-start": socket.emit("agent-turn-start", {}); break;
      ...
      case "call-summary": socket.emit("call-summary", event.data); break;
      case "recording-saved": socket.emit("recording-saved", event.data); break;
      case "mock-booking-created": socket.emit("mock-booking-created", event.data); break;
...
io.on("connection", (socket) => {
  ...
  socket.on("start-session", async (data) => {
    ...
    const pendingPrewarm = prewarmTimers.get(sessionId);
    if (pendingPrewarm) { clearTimeout(pendingPrewarm); prewarmTimers.delete(sessionId); }
  ...
  socket.on("disconnect", async () => {
    const pendingPrewarm = prewarmTimers.get(socket.id);
    if (pendingPrewarm) { clearTimeout(pendingPrewarm); prewarmTimers.delete(socket.id); }
    clearPrewarmState(socket.id);
    await closeSession(socket.id);
  });
});
...
app.get("/api/call-logs/export-csv", async (req, res) => {
  try {
    const logs = await CallLog.find().sort({ createdAt: -1 }).lean();
    const columns = ["id","sessionId","isSimulation","brand","caller_name","caller_phone","vehicle_interest","scenario","intent_category","outcome","upgrade_interest_level","recommended_next_action","sentiment","confidence_score","escalated","ai_summary","duration_seconds","createdAt"];
    const rows = logs.map((l) => ({ ...l, objections: undefined, booking: undefined, createdAt: l.createdAt ? new Date(l.createdAt).toISOString() : "" }));
    const csv = toCsv(rows, columns);
    res.setHeader("Content-Type", "text/csv");
    res.setHeader("Content-Disposition", `attachment; filename="pc-call-logs-${Date.now()}.csv"`);
    res.send(csv);
  } catch (err) {
    res.status(500).json({ error: "Failed to export call logs" });
  }
 ...
 * Core KPI summary for the dashboard header cards.
 * Includes: totalCalls, answeredCalls, answerRate, bookedCalls, conversionRate,
 * activeCampaigns (always 0 in sim mode — campaigns are separate), avgDuration,
 * escalations, positiveSentiment, totalBookings.
 */
app.get("/api/dashboard/stats", async (req, res) => {
  try {
    ...
    // "Answered" = any call that had a real conversation (not no_answer)
    const answered = logs.filter((l) => l.outcome !== "no_answer").length;
    const answerRate = total > 0 ? Math.round((answered / total) * 100) : 0;

    // Booked = service_booked + test_drive_booked
    const booked = logs.filter((l) =>
      ["service_booked", "test_drive_booked"].includes(l.outcome)
    ).length;
    // Conversion rate = booked / answered (not total, to be meaningful)
    const conversionRate = answered > 0 ? Math.round((booked / answered) * 100) : 0;
    ...
    // Average call duration (seconds) — only for calls that had a duration recorded
    const withDuration = logs.filter((l) => l.duration_seconds && l.duration_seconds > 0);
    const avgDuration = withDuration.length > 0
      ? Math.round(withDuration.reduce((sum, l) => sum + l.duration_seconds, 0) / withDuration.length)
      : 0;

    // Total confirmed bookings in the mock calendar
    const totalBookings = await MockBooking.countDocuments({ status: "confirmed" });

    // Callback rate
    const callbackScheduled = logs.filter((l) => l.outcome === "callback_scheduled").length;
    const callbackRate = total > 0 ? Math.round((callbackScheduled / total) * 100) : 0;

    // Upgrade interest: medium + high
    const upgradeInterest = logs.filter((l) =>
      ["medium", "high"].includes(l.upgrade_interest_level)
    ).length;
    ...
    res.json({
      ...
      callbackRate,
      upgradeInterest,
      activeCampaigns: 0, // populated by campaigns API if present
    });
  } catch (err) {
 ...
 * Day-by-day breakdown for the area chart (calls, answered, booked, duration).
 */
app.get("/api/dashboard/daily", async (req, res) => {
  try {
    ...
    const logs = await CallLog.find({ createdAt: { $gte: since } }).lean();
    ...
    for (let i = 0; i < days; i++) {
      ...
      byDay[key] = { date: key, calls: 0, answered: 0, booked: 0, declined: 0, noAnswer: 0, avgDuration: 0, _totalDuration: 0, _durationCount: 0 };
    }
    ...
    for (const log of logs) {
      ...
      byDay[key].calls += 1;
      if (log.outcome !== "no_answer") byDay[key].answered += 1;
      if (["service_booked", "test_drive_booked"].includes(log.outcome)) byDay[key].booked += 1;
      if (log.outcome === "declined") byDay[key].declined += 1;
      if (log.outcome === "no_answer") byDay[key].noAnswer += 1;
      if (log.duration_seconds > 0) {
        byDay[key]._totalDuration += log.duration_seconds;
        byDay[key]._durationCount += 1;
      }
    }
    ...
    const result = Object.values(byDay).map((d) => ({
      ...
      declined: d.declined,
      noAnswer: d.noAnswer,
      avgDuration: d._durationCount > 0 ? Math.round(d._totalDuration / d._durationCount) : 0,
      answerRate: d.calls > 0 ? Math.round((d.answered / d.calls) * 100) : 0,
      conversionRate: d.answered > 0 ? Math.round((d.booked / d.answered) * 100) : 0,
    }));

    res.json(result);
  } catch (err) {
    console.error("Dashboard daily error:", err.message);
 ...
 * Outcome distribution for the pie/donut chart.
 */
app.get("/api/dashboard/outcomes", async (req, res) => {
  try {
    ...
    res.json(result.map((r) => ({ outcome: r._id || "unknown", count: r.count })));
  } catch (err) {
 ...
 * Calls and conversion rate by scenario (for bar chart / table).
 */
app.get("/api/dashboard/scenarios", async (req, res) => {
  try {
    const result = await CallLog.aggregate([
      { $group: {
        _id: "$scenario",
        total: { $sum: 1 },
        booked: { $sum: { $cond: [{ $in: ["$outcome", ["service_booked", "test_drive_booked"]] }, 1, 0] } },
        answered: { $sum: { $cond: [{ $ne: ["$outcome", "no_answer"] }, 1, 0] } },
        positive: { $sum: { $cond: [{ $eq: ["$sentiment", "positive"] }, 1, 0] } },
      }},
      { $sort: { total: -1 } },
    ]);
    res.json(result.map((r) => ({
      scenario: r._id || "unknown",
      total: r.total,
      booked: r.booked,
      answered: r.answered,
      conversionRate: r.answered > 0 ? Math.round((r.booked / r.answered) * 100) : 0,
      positiveSentimentRate: r.total > 0 ? Math.round((r.positive / r.total) * 100) : 0,
    })));
  } catch (err) {
    res.status(500).json({ error: "Failed to fetch scenario breakdown" });
  }
});

/**
 * GET /api/dashboard/brands
 * Per-brand breakdown (Toyota / Mercedes-Benz / Isuzu).
 */
  ...
  try {
    const result = await CallLog.aggregate([
      { $group: {
        ...
        answered: { $sum: { $cond: [{ $ne: ["$outcome", "no_answer"] }, 1, 0] } },
        positive: { $sum: { $cond: [{ $eq: ["$sentiment", "positive"] }, 1, 0] } },
        avgDuration: { $avg: { $cond: [{ $gt: ["$duration_seconds", 0] }, "$duration_seconds", null] } },
      }},
      { $sort: { total: -1 } },
    ]);
    res.json(result.map((r) => ({
      ...
      positiveSentimentRate: r.total > 0 ? Math.round((r.positive / r.total) * 100) : 0,
      avgDurationSeconds: r.avgDuration ? Math.round(r.avgDuration) : 0,
    })));
  } catch (err) {
    res.status(500).json({ error: "Failed to fetch brand breakdown" });
  }
});

/**
 * GET /api/dashboard/sentiments
 * Sentiment distribution with a time series (last 30 days).
 */
app.get("/api/dashboard/sentiments", async (req, res) => {
  try {
    const overall = await CallLog.aggregate([
      { $group: { _id: "$sentiment", count: { $sum: 1 } } },
    ]);
    res.json({
      distribution: overall.map((r) => ({ sentiment: r._id || "neutral", count: r.count })),
    });
  } catch (err) {
...
/**
 * GET /api/dashboard/objections
 * Top objections raised by customers.
 */
app.get("/api/dashboard/objections", async (req, res) => {
  try {
    const logs = await CallLog.find({ objections: { $exists: true, $not: { $size: 0 } } }).lean();
    const counts = {};
    for (const log of logs) {
      for (const obj of (log.objections || [])) {
    ...
    res.status(500).json({ error: "Failed to fetch objection data" });
  }
});

/**
 * GET /api/dashboard/recent-calls?limit=10
 * Recent call log entries for the activity feed.
 */
app.get("/api/dashboard/recent-calls", async (req, res) => {
  try {
    const limit = Math.min(50, parseInt(req.query.limit) || 10);
    const logs = await CallLog.find()
      .sort({ createdAt: -1 })
      .limit(limit)
      .select("caller_name vehicle_interest outcome sentiment scenario brand duration_seconds ai_summary createdAt booking")
      .lean();
    res.json(logs);
  } catch (err) {
...
/**
 * GET /api/dashboard/upgrade-funnel
 * Upgrade interest funnel: none → low → medium → high.
 */
app.get("/api/dashboard/upgrade-funnel", async (req, res) => {
  try {
    const result = await CallLog.aggregate([
      { $group: { _id: "$upgrade_interest_level", count: { $sum: 1 } } },
    ]);
    const order = ["none", "low", "medium", "high"];
    const map = {};
    for (const r of result) { if (r._id) map[r._id] = r.count; }
    res.json(order.map((level) => ({ level, count: map[level] || 0 })));
  } catch (err) {
...
app.post("/api/session-summary/pdf", async (req, res) => {
  try {
    ...
    doc.fillColor("#0C1E3C").font("Helvetica-Bold").fontSize(20).text("Patterson Cheney", PAGE_LEFT, doc.y, { align: "left" });
    doc.fillColor("#666666").font("Helvetica").fontSize(12).text("Voice Session Summary", PAGE_LEFT, doc.y + 2, { align: "left" });
    doc.moveDown(0.8);
    doc.strokeColor("#0C1E3C").lineWidth(2).moveTo(PAGE_LEFT, doc.y).lineTo(PAGE_RIGHT, doc.y).stroke();
    doc.moveDown();

    const dateStr = new Date().toLocaleString();
    doc.fillColor("#000").font("Helvetica").fontSize(10).text(`Date: ${dateStr}`, PAGE_LEFT, doc.y);
    doc.text(`Duration: ${formatDurationForPdf(duration)}`, PAGE_LEFT, doc.y);
    doc.moveDown();

    const sectionHeader = (title) => {
      doc.x = PAGE_LEFT;
      doc.fillColor("#0C1E3C").font("Helvetica-Bold").fontSize(13).text(title, PAGE_LEFT, doc.y, { width: CONTENT_WIDTH, align: "left" });
      doc.moveDown(0.3);
      doc.strokeColor("#dddddd").lineWidth(1).moveTo(PAGE_LEFT, doc.y).lineTo(PAGE_RIGHT, doc.y).stroke();
      doc.moveDown(0.6);
    };

    const row = (label, value) => {
      const startY = doc.y;
      doc.fillColor("#666666").font("Helvetica-Bold").fontSize(10).text(label, PAGE_LEFT, startY, { width: LABEL_WIDTH, align: "left" });
      doc.fillColor("#000000").font("Helvetica").fontSize(10).text(value || "N/A", PAGE_LEFT + LABEL_WIDTH, startY, { width: CONTENT_WIDTH - LABEL_WIDTH, align: "left" });
      doc.moveDown(0.5);
    };
    ...
    if (callLog) {
      ...
      row("Outcome", humanize(callLog.outcome));
      row("Sentiment", humanize(callLog.sentiment));
      if (callLog.upgrade_interest_level) row("Upgrade Interest", humanize(callLog.upgrade_interest_level));
      if (callLog.recommended_next_action) row("Recommended Next Action", callLog.recommended_next_action);
      if (callLog.booking_ref) row("Booking Reference", callLog.booking_ref);
      if (Array.isArray(callLog.objections) && callLog.objections.length) row("Objections", callLog.objections.join(", "));
      doc.moveDown();
      sectionHeader("AI Summary");
      doc.fillColor("#333333").font("Helvetica").fontSize(10).text(callLog.ai_summary || "N/A", PAGE_LEFT, doc.y, { width: CONTENT_WIDTH, align: "left" });
      doc.moveDown();
    }

    if (!customer && !callLog) {
      doc.fillColor("#666666").font("Helvetica").fontSize(11).text("No session data was available to include in this report.", PAGE_LEFT, doc.y, { width: CONTENT_WIDTH, align: "left" });
    }

    doc.fillColor("#999999").font("Helvetica").fontSize(8).text("Generated by Patterson Cheney Voice Agent Simulation", PAGE_LEFT, doc.page.height - doc.page.margins.bottom - 15, { width: CONTENT_WIDTH, align: "center" });
    doc.end();
  } catch (err) {
    console.error("Session summary PDF generation failed:", err.message);
    if (!res.headersSent) res.status(500).json({ error: "Failed to generate PDF summary" });
  }
});

// ─── REST: Simulation Customers ───────────────────────────────────────────────
app.post("/api/simulation/customers/import-csv", upload.single("file"), async (req, res) => {
  try {
    ...
    if (rows.length === 0) return res.status(400).json({ error: "CSV had no data rows" });

    const requiredCols = ["name", "vehicle", "brand", "scenario"];
    const missingCols = requiredCols.filter((c) => !(c in rows[0]));
    if (missingCols.length) {
      return res.status(400).json({ error: `CSV missing required column(s): ${missingCols.join(", ")}`, expectedColumns: ["name","phone","vehicle","brand","suburb","scenario","lastService","upgradeScore"] });
    }

    const batchLabel = req.body.batchLabel || `import_${Date.now()}`;
    const validScenarios = ["service_due","upgrade_opportunity","finance_renewal","objection_handling","callback_follow_up"];
    const validBrands = ["Toyota", "Mercedes-Benz", "Isuzu"];
    ...
    rows.forEach((r, idx) => {
      const lineNo = idx + 2;
      if (!r.name || !r.vehicle) { errors.push(`Line ${lineNo}: missing name or vehicle`); return; }
      if (!validBrands.includes(r.brand)) { errors.push(`Line ${lineNo}: brand must be one of ${validBrands.join(", ")}`); return; }
      if (!validScenarios.includes(r.scenario)) { errors.push(`Line ${lineNo}: scenario must be one of ${validScenarios.join(", ")}`); return; }
      docs.push({ id: uuidv4(), name: r.name, phone: r.phone || null, vehicle: r.vehicle, brand: r.brand, suburb: r.suburb || null, scenario: r.scenario, lastService: r.lastService || null, upgradeScore: r.upgradeScore ? Number(r.upgradeScore) : 3, batchLabel });
    });

    if (docs.length) await SimulationCustomer.insertMany(docs);
    res.json({ imported: docs.length, skipped: errors.length, batchLabel, errors: errors.slice(0, 20) });
  } catch (err) {
    console.error("CSV import failed:", err.message);
    res.status(500).json({ error: "Failed to import CSV" });
  }
...
app.get("/api/simulation/customers", async (req, res) => {
  try {
    const filter = req.query.batchLabel ? { batchLabel: req.query.batchLabel } : {};
    const customers = await SimulationCustomer.find(filter).sort({ createdAt: -1 }).lean();
    res.json(customers);
  } catch (err) {
    res.status(500).json({ error: "Failed to fetch simulation customers" });
  }
...
app.get("/api/simulation/customers/export-csv", async (req, res) => {
  try {
    const filter = req.query.batchLabel ? { batchLabel: req.query.batchLabel } : {};
    const customers = await SimulationCustomer.find(filter).sort({ createdAt: -1 }).lean();
    const columns = ["name","phone","vehicle","brand","suburb","scenario","lastService","upgradeScore","batchLabel"];
    const csv = toCsv(customers, columns);
    res.setHeader("Content-Type", "text/csv");
    res.setHeader("Content-Disposition", `attachment; filename="pc-simulation-customers-${Date.now()}.csv"`);
    res.send(csv);
  } catch (err) {
    res.status(500).json({ error: "Failed to export simulation customers" });
  }
...
app.get("/api/simulation/customers/template-csv", (req, res) => {
  const csv = toCsv(
    [
      { name: "James Nguyen", phone: "0400000001", vehicle: "2021 Toyota HiLux SR5", brand: "Toyota", suburb: "Keysborough", scenario: "service_due", lastService: "2025-10-20", upgradeScore: 5 },
      { name: "Sarah Thompson", phone: "0400000002", vehicle: "2022 Mercedes GLC 300", brand: "Mercedes-Benz", suburb: "Berwick", scenario: "upgrade_opportunity", lastService: "2025-09-10", upgradeScore: 4 },
    ],
    ["name","phone","vehicle","brand","suburb","scenario","lastService","upgradeScore"],
  );
  res.setHeader("Content-Type", "text/csv");
  res.setHeader("Content-Disposition", 'attachment; filename="pc-simulation-customer-template.csv"');
  res.send(csv);
});

// ─── REST: Mock Booking Calendar ──────────────────────────────────────────────
app.get("/api/mock-bookings", async (req, res) => {
  try {
    const bookings = await MockBooking.find().sort({ date: 1, time: 1 }).lean();
    res.json(bookings);
...
app.post("/api/mock-bookings", async (req, res) => {
  try {
    const { customer_name, phone, vehicle, brand, type, date, time, notes } = req.body;
    if (!customer_name || !type || !date || !time) {
      return res.status(400).json({ error: "customer_name, type, date, time are required" });
    }
    const booking = await createMockBooking({ customer_name, phone, vehicle, brand, type, date, time, notes });
    res.json(booking);
  } catch (err) {
    res.status(500).json({ error: "Failed to create mock booking" });
  }
